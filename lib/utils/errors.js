...
exports.IndexingError = exports.SearchError = exports.ConfigurationError = exports.ParsingError = exports.FileAccessError = exports.DatabaseError = exports.ConversationSearchError = void 0;
exports.createUserFriendlyError = createUserFriendlyError;
exports.getErrorResponse = getErrorResponse;
class ConversationSearchError extends Error {
    code;
    userMessage;
    cause;
...
exports.ConversationSearchError = ConversationSearchError;
class DatabaseError extends ConversationSearchError {
...
class FileAccessError extends ConversationSearchError {
...
class ParsingError extends ConversationSearchError {
...
class ConfigurationError extends ConversationSearchError {
...
class SearchError extends ConversationSearchError {
...
class IndexingError extends ConversationSearchError {
    constructor(message, cause) {
...
function createUserFriendlyError(error, fallbackMessage = 'An unexpected error occurred') {
...
function getErrorResponse(error, context) {
    ...
    switch (searchError.code) {
        case 'FILE_ACCESS_ERROR':
            errorText.push('• Check if ~/.claude/projects directory exists', '• Verify Claude Code has read permissions', '• Ensure conversation files are not corrupted');
            break;
        case 'DATABASE_ERROR':
            errorText.push('• Try running refresh_index() to rebuild the database', '• Check if database file is not locked by another process', '• Verify disk space is available');
            break;
        case 'PARSING_ERROR':
            errorText.push('• Some conversation files may be corrupted', '• Try restarting Claude Code to regenerate files', '• Check if specific files can be manually opened');
            break;
        case 'CONFIG_ERROR':
            errorText.push('• Check MCP server configuration in claude_desktop_config.json', '• Verify environment variables are set correctly', '• Ensure proper file paths and permissions');
            break;
        case 'SEARCH_ERROR':
            errorText.push('• Try a simpler search query', '• Check if database is indexed (run refresh_index())', '• Verify search terms are not empty');
            break;
        case 'INDEXING_ERROR':
            errorText.push('• Check if ~/.claude/projects contains conversation files', '• Verify Claude Code is creating conversation files', '• Try restarting the MCP server');
            break;
        default:
            errorText.push('• Try restarting the MCP server', '• Check the console for detailed error logs', '• Verify Claude Code is working properly');
    }
    return {
        isError: true,
        content: [{
