...
export class ConversationDatabase {
    ...
    insertMessage(message) {
      ...
      `);
            stmt.run(message.id, message.conversationId, message.projectPath, message.projectName, message.timestamp.getTime(), message.type, message.content, JSON.stringify(message.rawContent), message.toolOperations ? JSON.stringify(message.toolOperations) : null, message.messageUuid, message.parentUuid);
        }
        catch (error) {
    ...
    getMessageContext(message, size) {
    ...
    updateIndexingMetadata(filePath, fileSize, messageCount) {
        const stmt = this.db.prepare(`
    ...
    isFileIndexed(filePath, fileSize) {
        const stmt = this.db.prepare(`
    ...
    getConversationMessages(conversationId, limit, startFrom) {
        let query;
        let params;
        if (startFrom >= 0) {
