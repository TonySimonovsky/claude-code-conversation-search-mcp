...
class ConversationSearchServer {
    ...
    constructor() {
        try {
            ...
            throw (0, errors_1.createUserFriendlyError)(error, 'Failed to initialize conversation search server. Check configuration and file permissions.');
        }
    }
    loadConfig() {
        const config = {
    ...
    validateConfig(config) {
        ...
        if (config.indexInterval !== undefined && (isNaN(config.indexInterval) || config.indexInterval < 0)) {
            throw new errors_1.ConfigurationError('INDEX_INTERVAL must be a positive number');
        }
        if (config.maxResults !== undefined && (isNaN(config.maxResults) || config.maxResults < 1 || config.maxResults > 1000)) {
            throw new errors_1.ConfigurationError('MAX_RESULTS must be between 1 and 1000');
        }
        if (config.defaultContextSize !== undefined && (isNaN(config.defaultContextSize) || config.defaultContextSize < 0 || config.defaultContextSize > 50)) {
            throw new errors_1.ConfigurationError('DEFAULT_CONTEXT_SIZE must be between 0 and 50');
        }
        if (config.searchTimeout !== undefined && (isNaN(config.searchTimeout) || config.searchTimeout < 1000 || config.searchTimeout > 300000)) {
            throw new errors_1.ConfigurationError('SEARCH_TIMEOUT must be between 1000ms and 300000ms (5 minutes)');
        }
        if (config.maxMemoryMB !== undefined && (isNaN(config.maxMemoryMB) || config.maxMemoryMB < 64 || config.maxMemoryMB > 8192)) {
            throw new errors_1.ConfigurationError('MAX_MEMORY_MB must be between 64MB and 8GB');
        }
        if (config.indexBatchSize !== undefined && (isNaN(config.indexBatchSize) || config.indexBatchSize < 1 || config.indexBatchSize > 10000)) {
            throw new errors_1.ConfigurationError('INDEX_BATCH_SIZE must be between 1 and 10000');
        }
        if (config.fullTextMinLength !== undefined && (isNaN(config.fullTextMinLength) || config.fullTextMinLength < 1 || config.fullTextMinLength > 50)) {
            throw new errors_1.ConfigurationError('FULL_TEXT_MIN_LENGTH must be between 1 and 50');
        }
        if (config.maxFileSize !== undefined && (isNaN(config.maxFileSize) || config.maxFileSize < 1024 || config.maxFileSize > 1073741824)) {
            throw new errors_1.ConfigurationError('MAX_FILE_SIZE must be between 1KB and 1GB');
        }
        if (config.indexThreads !== undefined && (isNaN(config.indexThreads) || config.indexThreads < 1 || config.indexThreads > 16)) {
            throw new errors_1.ConfigurationError('INDEX_THREADS must be between 1 and 16');
        }
        if (config.dbBackupInterval !== undefined && (isNaN(config.dbBackupInterval) || config.dbBackupInterval < 3600000)) {
            throw new errors_1.ConfigurationError('DB_BACKUP_INTERVAL must be at least 1 hour (3600000ms)');
        }
        // Validate log level
        const validLogLevels = ['error', 'warn', 'info', 'debug'];
        if (config.logLevel && !validLogLevels.includes(config.logLevel)) {
            throw new errors_1.ConfigurationError(`LOG_LEVEL must be one of: ${validLogLevels.join(', ')}`);
        }
        ...
        if (config.allowedFileExtensions) {
            ...
            if (invalidExts.length > 0) {
                throw new errors_1.ConfigurationError(`Invalid file extensions: ${invalidExts.join(', ')}. Extensions must start with '.' and be at least 2 characters`);
            }
        }
    }
    ...
    async searchConversations(args) {
        try {
            ...
            if (!query || typeof query !== 'string') {
                throw new errors_1.SearchError('empty query', new Error('Query parameter is required and must be a string'));
            }
            if (query.trim().length === 0) {
                throw new errors_1.SearchError('empty query', new Error('Search query cannot be empty'));
            }
            ...
            return (0, errors_1.getErrorResponse)(error, 'Search');
        }
    }
    async listProjects() {
        try {
            ...
            return (0, errors_1.getErrorResponse)(error, 'List projects');
        }
    }
    async getMessageContext(args) {
        try {
            ...
            if (!messageId || typeof messageId !== 'string') {
                throw new errors_1.SearchError('invalid message ID', new Error('Message ID is required and must be a string'));
            }
            ...
            return (0, errors_1.getErrorResponse)(error, 'Get message context');
        }
    }
    async getConversationMessages(args) {
        try {
            ...
            if (!conversationId || typeof conversationId !== 'string') {
                throw new errors_1.SearchError('invalid conversation ID', new Error('Conversation ID is required and must be a string'));
            }
            ...
            return (0, errors_1.getErrorResponse)(error, 'Get conversation messages');
        }
    }
    async listTools() {
        try {
            ...
            return (0, errors_1.getErrorResponse)(error, 'List tools');
        }
    }
    async getConfigInfo() {
        try {
            ...
            return (0, errors_1.getErrorResponse)(error, 'Get configuration info');
        }
    }
    async refreshIndex() {
        try {
            ...
            return (0, errors_1.getErrorResponse)(error, 'Index refresh');
        }
    }
    async run() {
        try {
